...
% \text{UnaryOperator} & \Acd{UnaryOp} & ::= & - & \text{(number negation)} \\
% &&|& ! & \text{(boolean not)}\\
% &&|& ~ & \text{(bit not)}\\
% &&|& \Acd{TypeOf} & \text{(typeof)}\\
% &&|& \Acd{Symbol} & \text{(symbol)}\\
% &&|& \Acd{Copy} & \text{(copy)}\\
% &&|& \Acd{GetSyntax} & \text{(get syntax)}\\
% &&|& \Acd{GetKeys} & \text{(get keys)}\\
% \\

% \text{BinaryOperator} & \Acd{BinaryOp} & ::= & + & \text{(add)}\\
% &&|& - & \text{(subtraction)}\\
% &&|& * & \text{(multiplication)}\\
% \\

% \text{ConvertOperator} & \Acd{ConvertOp} & ::= & \Acd{str2num} & \text{(string to number)}\\
% &&|& \Acd{num2str} & \text{(number to string)}\\
% &&|& \Acd{num2int} & \text{(number to integer)}\\
% \\
