...
special variant of EBNF with parametric non-terminals, conditional
...
\begin{itemize}[leftmargin=0.5cm]
  ...
  \item \textbf{ Using \( \tool \), we could define the \textit{first IR-based
    ...
    translator (\inred{XXX} steps) in \inred{XX} days.  We evaluated the correctness
    ...
    applicable tests.  Moreover, we found \inred{X} specification errors in ES10
  ...
  \item \textbf{\( \tool \) is also \textit{forward compatible} with new language features
    ...
    to the current draft of the next ECMAScript (ES11, 2020).  It successfully
    synthesized a parser and automatically
    compiled \inred{XX} out of \inred{XX} algorithm steps for ES11.
    After completion of the missing parts, the extracted semantics passed all \inred{XX} tests.
\end{itemize}
