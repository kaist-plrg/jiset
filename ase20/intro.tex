...
\begin{itemize}[leftmargin=0.5cm]
  \item \textbf{\( \tool \) is the \textit{first tool that automatically
    ...
    \inred{XX.X}\% of the steps in abstract algorithms on average.
  \item \textbf{\( \tool \) bridges gaps between tests and the specification
    written in a natural language.}
    Because the formal semantics automatically extracted by \( \tool \) is executable
    and its components have one-to-one correspondences with the components of
    ECMAScript, we can check the specification
    using the official test conformance suite, Test262~\cite{test262}.  After
    manually completing missing parts of the AST-IR translator for the latest
    ECMAScript (ES10, 2019), which is the \textit{first IR-based formal
    semantics of modern JavaScript}, we found \inred{X}
    specification errors in ES10 and \inred{XX} wrong tests in Test262. They
    were all confirmed by TC39 and will be fixed in the next release.  After
    fixing them, the formal semantics passed all \inred{XX,XXX} applicable
    tests.
  \item \textbf{\( \tool \) is also \textit{forward compatible} with new
    ...
    \inred{X} proposals that are ready for inclusion in the next ECMAScript
    (ES11, 2020).  It automatically synthesized parsers and compiled \inred{XXX}
    out of \inred{XXX} algorithm steps for all the proposals.  After completion
    of the missing parts, we found \inred{X} specifications errors by executing
    the corresponding tests in Test262.  After fixing them, the extracted
    semantics passed all \inred{XXX} tests.
\end{itemize}
