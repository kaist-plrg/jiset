...
\code{A | B} of two parsing rules \code{A} and \code{B} parses the input using
...
several helper functions in parsing rules such as optional rule \code{A?},
positive(negative) predicate \code{+A}(\code{-A}).  For instance, the helper
...
\ires \) instruction whose beautified form is the sixth line of the function
body in Figure~\ref{fig:array-literal-eval}.
...
Thus, the corresponding \( \ires \) function has two parameters, the special
parameter \( \code{this} \) and normal one \( \code{N} \) and the method call
