...
the success rate of \inred{91.38\%} on average for each ECMAScript version
from ES7 to ES10, and \inred{92.08\%} for each update between adjacent
...
in ES10, \inred{8,901} steps out of \inred{9,674} steps are
automatically compiled by \textsf{Algorithm Compiler}.
Among the remaining \inred{773} steps, we manually implemented all
missing steps for the core language semantics (\inred{221} steps) and
the essential parts of the built-in libraries (\inred{XX} steps out of
\inred{552}).  We also manually implemented \textsf{Global Setting}
...
Among \inred{XX,XXX} tests in Test262, we filtered out \inred{XX,XXX}
tests as summarized in Table~\ref{table:test262}.  To focus on ES10,
we excluded \inred{X,XXX} tests for annexes, internationalization, and
in-progress features.  We also filted out \inred{X,XXX} tests that use
minor language features.
Finally, for \inred{XX,XXX} applicable tests, the extracted semantics
failed for \inred{XXX} tests.
...
It caused \inred{XX} tests failed.  We proposed a fix, which is
...
introduced on February 27, 2019 and caused \inred{XX} tests failed.
...
intended, \inred{XX} tests fail when a wrong semantics is chosen.
...
from the revised ES10 passed all \inred{XX,XXX} applicable tests in Test262,
...
\footnotesize
    \scode{matchAll} of \scode{String} &
    0 &
    0 &
    9/9 &
    5/5 &
...
to the proposals that are ready for inclusion in ES11.  Because
...
Otherwise, the committee changes its stage to Inactive.
Thus, we applied \( \tool \) to all nine proposals in the Finished stage.

Table~\ref{table:spec-prop-result} shows the result of applying \( \tool \) to the
Finished proposals.  Collectively, the proposals modified \inred{XX} lexical and
\inred{XX} syntactic productions, and \( \tool \) successfully
synthesized parsers for them.  The newly synthesized parsers parse all
...
Using the failed tests, we discovered four specification errors in the
...
All of them were confirmed by TC39 and will be fixed in the next ECMAScript.
...
The error BigInt-1 is due to a simple misuse of the variable 
\code{oldValue} in {\bf Evaluation} of {\it UpdateExpression}.
BigInt-2 and BigInt-3 break the backward compatibility because of
the incorrectly mixed uses of {\bf ToInt32} and {\bf ToUint32}
in unsigned right shift operators and bitwise operators, respectively.
Finally, BigInt-4 happened because the proposal did not consider
BigInt primitives in the Number constructor.  On average, four
specification errors existed for \inred{XXX} days in the proposal.
