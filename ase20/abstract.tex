...
which made already five releases after ES5.1.
...
ECMAScript.  For the five most recent ECMAScript versions and the
current draft of the next ECMAScript (ES11, 2020), \( \tool \) automatically
...
the extracted core semantics of ES11 passed all \inred{XX,XXX} applicable tests.
Using this \textit{first formal semantics of modern JavaScript}, we found
\inred{X} specification errors in ES11, which were all confirmed by
the Ecma Technical Committee 39.  Furthermore, we also successfully
applied \( \tool \) to \inred{X} feature proposals that are candidates
for inclusion in ECMAScript, which shows the forward compatibility of \( \tool \).
\end{abstract}
