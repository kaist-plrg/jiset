...
algorithms for Context-Free Grammar (CFG): JavaCC with LL(k)~\inred{\cite{llk}},
Bison with GLR~\inred{\cite{glr}}, and ANTLR with
ALL(*)~\inred{\cite{all-star}}.  However, they are not directly applicable for
...
the PEG in Equation~(1): %\ref{eq:simple-js}:
\begin{center}
...
Packrat parsing~\cite{packrat} proposed by Ford et al.  \inred{Moreover, we also
implemented the \textit{growing the seed} technique presented by Warth et
al~\cite{packrat-lr} to support direct and even indirect left recursive
productions.  It helps to synthesize parsers without changing the structure of
each production in \( \bnfes \).}
