package kr.ac.kaist.jiset.checker

import kr.ac.kaist.jiset.spec._
import kr.ac.kaist.jiset.spec.algorithm.Algo
import kr.ac.kaist.ires.ir._

object UnusedVarChecker extends Checker {
  // for specifications
  def apply(
    spec: ECMAScript,
    targets: List[Algo]
  ): List[UnusedVariable] = targets.flatMap(UnusedVarChecker(spec.globals, _))

  // for algorithms
  def apply(initial: Set[String], algo: Algo): Option[UnusedVariable] = {
    var scopeVars: Map[String, Boolean] = algo.params.map(_.name -> false).toMap
    object Walker extends UnitWalker {
      override def walk(inst: Inst): Unit = inst match {
        case ILet(x, e) =>
          walk(e); scopeVars += x.name -> false
        case IApp(x, f, as) =>
          walk(f); walkList[Expr](as, walk); scopeVars += x.name -> false
        case IAccess(x, b, e) =>
          walk(b); walk(e); scopeVars += x.name -> false
        case IWithCont(x, ps, b) =>
          walkList[Id](ps, walk); walk(b); scopeVars += x.name -> false
        case _ => super.walk(inst)
      }
      override def walk(id: Id): Unit = {
        val name = id.name
        if (scopeVars.keySet.exists(_ == name)) {
          scopeVars += name -> true
        }
      }
    }
    Walker.walk(algo.body)
    val errors = scopeVars.filter(t => !t._2).keySet
    if (errors.isEmpty) None
    else Some(UnusedVariable(algo, errors))
  }
}
