...
import kr.ac.kaist.jiset.ir.{ REPL => IRREPL, _ }
...
object AbsTransfer {
  ...
  final def compute: Unit = worklist.next match {
    case Some(cp) =>
      ...
      AnalysisStat.iter += 1
      if (LOG && AnalysisStat.iter % 10000 == 0) AnalysisStat.dump()
      compute
    case None =>
      alarmCP = null
      alarmCPStr = ""
  ...
  class Helper(ret: ReturnPoint) extends PruneHelper {
    ...
    val predAlgos: Map[String, PredAlgo] = Map(
      ...
      arityCheck("fround", {
        case (_, List((expr, ty))) =>
          AnalysisStat.doCheck(if (!(ty ⊑ NumT)) alarm(s"non-number types: ${expr.beautified}"))
          NumT
      }),
