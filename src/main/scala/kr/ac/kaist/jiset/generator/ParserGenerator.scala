...
case class ParserGenerator(grammar: Grammar, modelDir: String) {
  val Grammar(lexProds, pords) = grammar
  ...
  // XXX REMOVE because it is temporal
  nf.println(s"""  val Script: ESParser[Script] = ???""")
  nf.println(s"""  val rules: Map[String, ESParser[AST]] = ???""")
  nf.println(s"""  protected val TERMINAL: Lexer = ???""")
