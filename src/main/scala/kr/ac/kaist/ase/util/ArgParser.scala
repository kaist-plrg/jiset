...
import kr.ac.kaist.ase.{ ASE, Command, ASEConfig }
import kr.ac.kaist.ase.phase.{ PhaseOption, Config }
import kr.ac.kaist.ase.error._
import spray.json._

// Argument parser by using Scala RegexParsers.
class ArgParser(cmd: Command, aseConfig: ASEConfig) extends RegexParsers {
  var ruleList: List[Parser[Unit]] = Nil

  var optNameSet: Set[String] = Set()

  addRule(aseConfig, "", ASE.options)
  ...
  def apply(args: List[String]): Unit = {
        ...
        Source.fromFile(fileName)("UTF-8").mkString.parseJson match {
          case (obj: JsObject) => obj.fields.foreach {
            case (phase, value: JsObject) => {
              if (ASE.phases.map(_.name).contains(phase))
    ...
    lazy val fileName: Parser[Unit] = str ^^ {
      s => aseConfig.fileNames = s :: aseConfig.fileNames
    }
    ...
    aseConfig.fileNames = aseConfig.fileNames.reverse
  }
}
