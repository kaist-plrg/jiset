...
object AlgoCompiler extends TokenParsers {
  ...
  def letStmt = ("let" ~> id <~ "be") ~ (expr <~ ("."?) <~ next) ^^ {
    case e1 ~ e2 => ILet(Id(e1), e2)
  }
  ...
  def returnStmt = "return" ~> expr <~ ("."?) <~ next ^^ {
    case e1 => IReturn(e1)
  }
  ...
  def evaluationExpr = ("the result of evaluating" ~> word) ^^ {
    case s => ERun(ERef(RefId(Id(s))), "Evaluation", Nil)
  }
