...
\begin{figure}[t]
  ...
  \end{subfigure}
  \caption{Number of steps for abstract algorithms in specifications, from which
...
syntactic grammar productions are 79.2 and 168, respectively.
In addition, the average numbers of annually updated lexical and
syntactic grammar productions between adjacent versions are 4.7
and 55, respectively.  We report that \( \tool \)
automatically generated parsers for \textit{all} the productions.

For semantics, we measured the number of steps in abstract algorithms that
...
automatically extract semantics with the 91.60\% success rate on
average for existing specifications, and 90.43\% for changed or
newly defined steps.  Moreover, we found that algorithm steps in
built-in libraries are less covered than other language semantics
with 86.98\% success rates (86.09\% for updates).
It is due to the diverse writing styles to represent
their own specific functionalities.
However, steps in core language semantics are covered
with higher success rates 95.35\% (95.25\% for updates).

According to this evaluation, we believe that \( \tool \) effectively
