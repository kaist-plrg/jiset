\begin{abstract}\label{sec:abstract}
  ...
  complex semantics in newly introduced language features and desire to
  understand them correctly.  To meet the demand, researchers have proposed
  several tools that enhance the understanding of JavaScript semantics. However,
  existing tools 1) require manual updates for new specifications or 2) do not
  provide any automatic ways to focus on specific semantics.
  ...
  tool offers two approaches to focus on target semantics: 1) reducing semantics
  with a user-defined \textit{syntactic view} and 2) providing a small-sized
  program related to a selected algorithm step with automatically inferred
  assertions. For evaluation, we experimentally demonstrate the effectiveness of
