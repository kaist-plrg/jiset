...
values are defined in the corresponding tables.  For example, fields of the
``Realm'' record type are defined in ``Table 23: Realm Record Fields'' in
ECMAScript.  The latests version of ECMAScriptThe contains tens of tables for
record types and they are backward compatible in general.  Thus, we manually
model them to use them in type analysis.  For lists, we define the empty list
type $\clist{}$ and parametric list types $\clist{\tau}$.
