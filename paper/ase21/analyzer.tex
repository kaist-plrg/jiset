...
sensitivity~\cite{sens-toplas}.  We extend types as follows:
\begin{figure}[H]
  \centering
  \vspace*{-0.5em}
  ...
  \vspace*{-0.5em}
\end{figure} \noindent
with function types $\func$, constant types $\const$, and
the absent type $\tabsent$ representing the absence of variables.
\jscode{Boolean} values $\bool$, \jscode{String} values $\str$,
$\tnormal(\ty)$ denoting normal completions with \code{Value} fields of type $\ty$,
and $\tabrupt$ denoting abrupt completions serve to improve the analysis precision.
  ...
  \resizebox{0.9\columnwidth}{!}{$
    \pass(\expr, \bool)(\aenv) = \left\{
      \begin{array}{ll}
        ...
        & \text{if} \; \bool \order \aseme{\expr}(\aenv)\\
...
\begin{figure}[H]
  ...
  \resizebox{\columnwidth}{!}{$
    \begin{array}{@{}r@{~}c@{~}l@{}}
      ...
      \aenv[ \x \mapsto \aty_\x \meet \tnormal(\tyset) ]\\
      \refine(\x.\Type \; \code{==} \; \nconst{normal}, \false)(\aenv) &=&
      \aenv[ \x \mapsto \aty_\x \meet \{ \tabrupt \} ]\\
