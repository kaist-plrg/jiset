...
\inred{1,689.6} functions on average.  Since ECMAScript has gradually evolved,
it analyzed \inred{1,490} functions for the first version in 2018
but analyzed \inred{1,863} functions in the latest.
$\tool$ analyzes functions with flow- and type-sensitive views.
On average, each version has \inred{91.2}K views and each function has
\inred{53.9}~views.
...
$\tool$ took \inred{136.0} seconds with \inred{298.8}K iterations of
...
\inred{127.0} seconds for the type analysis (\stextsf{analyze}), and
\inred{0.7} seconds for the bug detection (\stextsf{detect}).
...
is \inred{52.0}\%; \inred{92} out of \inred{177} detected bugs are true bugs.
The reference checker detected the most bugs with \inred{62.2}\% precision;
\inred{17} unknown variables (\stextsf{UnknownVar}) and
\inred{44} deplicated variable declarations (\stextsf{DuplicatedVar}) are true bugs.
We found \inred{4} missing parameters (\stextsf{MissingParam}) with \inred{100.0}\% precision and
\inred{5} assertion failures (\stextsf{Assertion}) with \inred{16.1}\% precision.
Finally, the operand checker detected
\inred{2} non-numeric operand bugs (\stextsf{NoNumber}) with \inred{33.3}\% precision and
...
Among \inred{92} true bugs, \inred{47} bugs are \textit{inherited},
...
Even though we assume that \inred{47} inherited bugs are created on January 1, 2018,
the average TTL is \inred{409.5} and the maximum TTL is \inred{1,164}.
All the bugs with the maximum TTL are inherited ones and they are all newly detected.

We manually investigated \inred{85} false-positive bugs to understand why
$\tool$ detected them.  Among them, \inred{30} bugs are due to extraction failure
...
to false-positive bugs.  The remaining \inred{55} bugs are due to imprecise analysis.
We found that \inred{20} bugs are due to imprecise analysis of the
...
Without refinement, the type analysis took \inred{90.9} seconds with \inred{259.1}K
...
the analysis time is increased at least \inred{1.05}x, at most \inred{1.98}x,
...
(\stextsf{Assertion}) by removing \inred{28} and \inred{26} false-positive bugs, respectively.
...
Among \inred{92} true bugs detected by $\tool$, \inred{14} are newly
