...
For example, if the variable $\x$ has a normal completion with \code{42} as its
actual value, $\code{x + 1}$ should be \code{43} because the normal completion
gets implicitly converted into its actual value \code{42}.
...
Then, an unchecked abrupt completion bug (\stextsf{Abrupt}) occurs
