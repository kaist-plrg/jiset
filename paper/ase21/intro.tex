...
information directly extracted from specifications written in a structured
natural language are recently presented in diverse fields such as system
...
intermediate representation for ECMAScript.  Therefore, we leverage this tool to
...
3) operator type checker, and 4) assertion checker.  Moreover, to increase
...
analysis.  We evaluate our tool with all \inred{XXX} different versions existed
in the official ECMAScript repository for the recent \inred{three years from
2018 to 2021}.
...
\begin{itemize}
  \item We present $\tool$, which is the first tool that performs \textit{type
    ...
    checker, 2) arity checker, 3) operator type checker, and 4) assertion
    checker.
