...
let PropertyDefinition = (Yield, Await) => (given) => {
  ...
  } else if (type === 'RestElement' || type === 'SpreadElement') {
    let { argument } = given;
    let expr = AssignmentExpression(true, Yield, Await)(argument);
    return new Node('PropertyDefinition', given, 4, [expr], params);
  } else {
    Node.TODO(`${given.type} @ PropertyDefinition`);
  }
}
