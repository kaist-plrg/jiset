...
let MemberExpression = (Yield, Await) => (given) => {
  ...
  switch (type) {
    ...
    case 'TaggedTemplateExpression': {
      let member = MemberExpression(Yield, Await)(given.tag);
      let temp = TemplateLiteral(Yield, Await, true)(given.quasi);
      return new Node('MemberExpression', given, 3, [member, temp], params);
    }
    case 'NewExpression': {
    ...
    case 'MemberExpression': {
      ...
      if (meta) {
        ...
        let member = MemberExpression(Yield, Await)(object);
        let name = new LexicalNode('IdentifierName', property.name);
