Spec = (() => {
    ...
    static ES2018() {
      ...
      for (let prod of grammar.prods) {
        ...
        for (let rhs of prod.rhsList) {
          semMap[`${lhs}:${rhs}`] = { idx: i, sem: rhs.semantics };
          i++;
        }
      }
      let grammarSections = [
            ...
            if (grammar.tagName == 'EMU-GRAMMAR') {
              ...
              for (let prod of grammar.getElementsByTagName('emu-production')) {
                for (let rhs of prod.getElementsByTagName('emu-rhs')) {
                  let rule = {};
                  rule.prod = prod.getAttribute('name');
                  rule.rhs = rhs.innerText.replace(/\s+/g, '');
                  rules.push(rule);
                }
              }
              // TODO
  ...
  function getHead(algo) {
    ...
    let arr = [];
    for (x of str.matchAll(/[^ ,()\[\]]+/g)) arr.push(x[0]);
  ...
  class ButNotToken extends Token {
    ...
    toString() { return `${this.name} but not ${this.cases.join(' or ')}` }
  }
