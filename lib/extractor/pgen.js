Synthesizer = (() => {
  ...
  class Grammar {
    ...
    getAST() {
      let code = '';
      code += 'package kr.ac.kaist.ase.node.ast\n\n';
      code += 'trait AST\n\n';
      code += this.prods.map(prod => prod.getAST(this.lexNames)).join('');
      return code;
    }
  ...
  class Prod {
    ...
    getAST(lexNames) {
      ...
      let type = lhs.name;
      let code = `trait ${type} extends AST` + '\n';
      for (let i = 0; i < rhsList.length; i++) {
        let params = rhsList[i].getASTParams(lexNames);
  ...
  let parserStartCode = `package kr.ac.kaist.ase.parser
