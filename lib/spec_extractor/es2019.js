SpecExtractor.loadSpec(() => {
  with(SpecExtractor) {
    ...
    function addMethod(name, length, data) {
      ...
      } else if (name == 'StringGetOwnProperty') {
        data.steps[6].tokens[2] = 'is';
      ...
      } else if (name == 'AbstractEqualityComparison') { // specError #14
        data.params = ["x", "y"];
        data.steps[7] = {
    ...
    for (let id of grammarSections) {
      ...
      for (let elem of section.getElementsByTagName('emu-alg')) {
        ...
        if (algo) {
          ...
          let moreParams = [];
          if (algo.name == "StatementRules" || algo.name == "ExpressionRules") {
              algo.name = "HasCallInTailPosition";
