SpecExtractor.loadSpec(() => {
  with(SpecExtractor) {
      ...
      if (methodSet.has(name)) {
        switch (name) {
          ...
          case 'MakeArgSetter':
            ...
            break;
          default:
            console.error(`[AlreadyExist] ${name}`);
            return;
        }
      }
      methodSet.add(name);
    ...
    function addProperty(path, body) {
      let obj = global;
      for (let p of path) {
        let value = obj.Value;
        if (!(p in obj.Value)) value[p] = { Value: {} };
        obj = value[p];
      }
      obj.Value = body;
    }
      ...
      for (let elem of getElem(id).getElementsByTagName('emu-alg')) {
        ...
        if (algo) {
          switch (algo.body.kind) {
            ...
            //     }
            //   }
            //   addProperty(path, algo.body);
            //   break;
            case 'RuntimeSemantics':
              addMethod(algo.name, algo.body);
              break;
            default:
              // console.error(`[NotYetHandle] ${algo.body.kind}`);
          }
        } else {
          console.error(elem);
        }
