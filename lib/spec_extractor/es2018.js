  ...
  with(SpecExtractor) {
    ...
    function addMethod(name, data) {
      ...
      if (name == 'GLOBAL.Array.prototype.sort'
        ...
        addMethod('BindingIdentifier0IteratorBindingInitialization0', data)
        data.params = ['BindingIdentifier', 'iteratorRecord', 'environment'];
        addMethod('BindingIdentifier1IteratorBindingInitialization0', data)
        addMethod('BindingIdentifier2IteratorBindingInitialization0', data)
        data.params = params;
      } else if (
        name == 'GLOBAL.CreateStringIterator' ||
        name == 'GLOBAL.CreateMapIterator' ||
        name == 'GLOBAL.CreateSetIterator'
      ) {
        name = name.slice('GLOBAL.'.length);
      } else if (name == 'GLOBAL.Array.prototype.sort') {
        return;
      } else if (name == 'GLOBAL.Function.prototype.toString') {
        data.steps = [{
    ...
    for (let i = 1; i < intrinsicsList.length; i++) {
      ...
      if (property != '') intrinsics[intrinsic] = `GLOBAL.${property}`;
    }
          ...
          if (algo.name.startsWith('Propertiesof')) {
            ...
            case 'Method':
              ...
              for (let i = 0; i < params.length; i++) {
                ...
                if (!param.startsWith('...')) {
                  steps.push({
                    tokens: ['Let', {'id': param}, 'be', {'id': 'argumentsList'}, '[', i + '', ']', '.']
                  })
                }
              }
