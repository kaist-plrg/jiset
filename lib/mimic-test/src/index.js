...
let MODELDIR = `${process.env.JISET_HOME}/lib/mimic-test/resources/mimic-model/`;
...
// // foldername of the result
// function getResultFolderName(tokens) {
//   return getFolderName(RESULTDIR,tokens);
// }
...
function test(target, mimic) {
  ...
  filenames.forEach(filename => {
    ...
    if (shellstr.code === 0) {
      success_array.push(`${TEST262DIR}${filename}`);
      log_array.push(`${TEST262DIR}${filename}`);
    } else {
      fail_array.push(`${TEST262DIR}${filename}`);
      log_array.push(`${TEST262DIR}${filename}`);
