SpecExtractor = (() => {
          ...
          if (name in grammar.idxMap) {
            ...
            for (let token of rhs.tokens) {
              if (token instanceof Nonterminal)
                subs.push(token.name);
            }
            let subName = lhsName + grammar.idxMap[name].idx;
  ...
  // get Rhs case names
  function getRhsCaseName(lhsName, rhs) {
    let name = lhsName + ':';
    for (let token of rhs.tokens) {
      if (token instanceof Terminal) {
        ...
        name += token.name;
      } else if (token instanceof ButNotToken) {
        name += token.base.name + "butnot" + token.cases.map((x) => x.name).join('');
      }
    }
    return norm(name);
  }
